...
router.post('/request-otp', async (req,res)=>{
    ...
    if (student.hasSetPassword) return res.status(400).json({ success : false, message: 'password already set'});
    ...
    let transporter = nodemailer.createTransport({
        ...
        res.json({success : true, message: 'OTP send to mail'});
});
...
router.post('/login', async (req,res)=>{
    ...
    if (!student || !student.hasSetPassword ){
        return res.status(400).json({ success : false, message : 'invalid credentials or not activated'})
    }
